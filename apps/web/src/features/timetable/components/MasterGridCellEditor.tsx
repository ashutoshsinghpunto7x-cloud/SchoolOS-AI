...
import type { PeriodSlot, MasterGridCell } from '@schoolos/types';
import { useSetMasterGridCell } from '../hooks/useTimetable';
import { useTeachersPaginated } from '@/features/teachers/hooks/useTeachers';

interface MasterGridCellEditorProps {
  klass: string;
  section: string;
  academicYear: string;
  term?: string;
  slot: PeriodSlot;
  cell: MasterGridCell | null;
  onClose: () => void;
}
...
/**
 * Editor for one master-grid cell (Class×Section × Period). Deliberately
 * mirrors EntryEditDrawer's shape — same teacher-search pattern, same
 * conflict-aware save path — since a save here just fans the entry out
 * across the period's applicable weekdays server-side (see
 * timetableService.setMasterGridCell) rather than needing its own logic.
 */
export const MasterGridCellEditor = ({
  klass, section, academicYear, term, slot, cell, onClose,
}: MasterGridCellEditorProps) => {
  const { mutate: save, isPending: saving, error: saveError } = useSetMasterGridCell();
  ...
  const [teacherQuery, setTeacherQuery] = useState(cell?.teacherName ?? '');
  ...
  useEffect(() => {
    ...
    setTeacherQuery(cell?.teacherName ?? '');
  }, [cell]);
  ...
  function handleSave(ev: React.FormEvent) {
    ...
    save(
      {
        class: klass, section, academicYear, term,
        slotId:      slot._id,
  ...
  function handleClear() {
    save(
      { class: klass, section, academicYear, term, slotId: slot._id, subjectName: '' },
      { onSuccess: onClose },
    );
  }
      ...
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--tt-border)]">
          <div>
            <h2 className="text-base font-bold text-[var(--tt-text-primary)]">
              {cell ? 'Edit Period' : 'Assign Period'}
            </h2>
            <p className="text-xs text-[var(--tt-text-secondary)] mt-0.5">
              Class {klass}-{section} · {slot.name} ({slot.startTime}–{slot.endTime})
            </p>
          </div>
          <button
        ...
        <form onSubmit={handleSave} className="flex-1 flex flex-col gap-4 px-5 py-5">
          <div className="flex flex-col gap-1.5">
            ...
            <input value={form.subjectName} onChange={set('subjectName')} className={inputCls} placeholder="e.g. Mathematics" required />
          </div>
          ...
          <div className="flex flex-col gap-1.5 relative">
            ...
            <div className="relative">
              ...
              <input
                ...
                autoComplete="off"
              />
              {form.teacherId && <Check className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#2ED47A]" />}
            </div>
            <p className="text-xs text-[var(--tt-text-muted)]">Applied to every day this period runs — this is one fixed weekly pattern, not a per-day schedule.</p>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <input value={form.roomNumber} onChange={set('roomNumber')} className={inputCls} placeholder="e.g. Room 12, Lab A" />
          </div>
          ...
          <div className="flex gap-3 mt-auto pt-4 border-t border-[var(--tt-border)]">
            {cell && (
              <button
                ...
                className="h-11 px-4 rounded-xl border border-[#FF5B6A]/25 bg-[#FF5B6A]/10 hover:bg-[#FF5B6A]/20
                           flex items-center gap-1.5 text-sm font-semibold text-[#FF5B6A] transition-colors disabled:opacity-50"
              >
            ...
            <button
              type="submit" disabled={saving || !form.subjectName.trim()}
