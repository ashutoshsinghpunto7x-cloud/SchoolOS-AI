...
import { useReportCardRoster } from '../hooks/useReportCard';
...
export function ReportCardRosterPage() {
  const { examId = '', cls = '', section = '' } = useParams();
  const navigate = useNavigate();
  const { data, isLoading, isError } = useReportCardRoster(examId, cls, section);
  ...
  return (
    <div className="min-h-screen bg-[#FAFBFF] dark:bg-transparent">
      <div className="px-5 pt-6 pb-4 max-w-2xl mx-auto">
        ...
        <p className="text-sm text-gray-500 dark:text-white/40 mt-1">{data?.exam.name ?? 'Loading exam…'} · Pick a student to generate their report card.</p>
        ...
        <div className="flex flex-col gap-3 mt-6">
          {isLoading ? (
            ...
            <div className="bg-red-50 border border-red-100 rounded-2xl p-5 flex items-start gap-3">
              ...
              <p className="text-sm font-semibold text-red-700">Failed to load class roster</p>
            </div>
            ...
            data!.rows.map((row) => (
              <button
                ...
                onClick={() => navigate(`/teacher/report-cards/${examId}/student/${row.studentId}`)}
              ...
              >
                ...
                {row.missingSubjects > 0 ? (
                  <span className="flex items-center gap-1 text-[10px] font-semibold text-amber-600 bg-amber-50 rounded-full px-2 py-1 shrink-0">
                    <AlertTriangle className="w-3 h-3" /> {row.missingSubjects} missing
                  </span>
                ) : row.hasReportCard ? (
